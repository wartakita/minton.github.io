...
function notify(message) {
    if (Notification.permission === 'granted') {
        new Notification('PecintaMinton', {
            body: message
        });
    }
}
...
function normalizeStatus(value) {
    ...
    if (val.includes('completed') || val.includes('selesai') || val.includes('finished') || val.includes('final')) return 'Completed';
    if (val.includes('running') || val.includes('berlangsung') || val.includes('sedang') || val.includes('in progress') || val.includes('live')) return 'Running';
    if (val.includes('not started') || val.includes('belum') || val.includes('tidak dimulai') || val.includes('upcoming') || val.includes('upcoming match') || val.includes('akan')) return 'Not Started';
    return 'Other';
}
...
function createMatchElement(match) {
    const localTime = new Date(match.matchTime).toLocaleString(undefined, {
        dateStyle: 'medium',
        timeStyle: 'short',
    ...
    <div class="match-header">
      ...
      <div><strong>Status:</strong> ${normalizeStatus(match.matchStatusValue) === 'Not Started' ? 'Belum Mulai' : normalizeStatus(match.matchStatusValue)}</div>
...
async function init() {
    ...
    renderMatches(allMatches);
    setInterval(updateMatchesLoop, 5000);
}
